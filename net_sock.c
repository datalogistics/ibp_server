...
log_printf(10, "sock_io_wait: fd=%d tm=" TT " mode=%d state=%d errno=%d\n", pfd.fd, tm, mode, state, errno);
...
apr_size_t my_read(network_sock_t *sock, void *buf, apr_size_t *count)
{
   apr_size_t n;
...
//getpeername(s->fd, (struct sockaddr *)(&sa), &dummy);
//int port = ntohs(sa.sin_port);
//log_printf(10, "my_read: fd=%d errno=%d n=" I64T " want=" I64T " port=%d\n", s->fd, errno, n, *count, port);
log_printf(10, "my_read: fd=%d errno=%d n=" I64T " want=" I64T "\n", s->fd, errno, n, *count);
...
apr_size_t my_write(network_sock_t *sock, const void *buf, apr_size_t *count)
{
   apr_size_t n;
   sock_apr_overlay_t *s = (sock_apr_overlay_t *)(sock->fd);

   do {
      n = write(s->fd, buf, (*count));
   } while ((n==-1) && (errno==EINTR));

log_printf(10, "my_write: fd=%d errno=%d n=" I64T " want=" I64T "\n", s->fd, errno, n, *count);
...
//if (sock == NULL) log_printf(15, "sock_write: sock == NULL\n");
  ...
  //**QWERT
//Q  apr_thread_mutex_lock(sock->lock);
  //**QWERTY

//  end_time = apr_time_now() + tm;

  nbytes = count;
  err = my_write(sock, buf, &nbytes);
  if (err != APR_SUCCESS) {
...
//  eno = errno;
  ...
  //**QWERT
//Q  apr_thread_mutex_unlock(sock->lock);
  //**QWERTY
...
//  eno = errno;
  ...
  //**QWERT
//@  apr_thread_mutex_unlock(sock->lock);
  //**QWERTY
