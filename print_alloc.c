...
void print_rw_history(char *buffer, int *used, int nbytes, Allocation_rw_ts_t *ts_list, int start)
{
  ...
  for (i=0; i<ALLOC_HISTORY; i++) {
     ...
     if (t != 0) {
        apr_ctime(print_time, t);
        address2ipdecstr(hostip, ts->ts.host.ip, ts->ts.host.atype);
        append_printf(buffer, used, nbytes, "   " TT " * %s * %s * " LU " * " LU " * " LU "\n", t, print_time, hostip, ts->id, ts->offset, ts->size);
     }
  }
}
      ...
      int state, int cs_type, osd_off_t hbs, osd_off_t bs)
{
  ...
  if (apr_time_now() > t) {
     ...
     append_printf(buffer, used, nbytes, "Expiration: " TT " -- %s\n", t, print_time);
  }

  append_printf(buffer, used, nbytes, "is_alias: %d\n", a->is_alias);
  append_printf(buffer, used, nbytes, "Read cap: %s\n", a->caps[READ_CAP].v);
  append_printf(buffer, used, nbytes, "Write cap: %s\n", a->caps[WRITE_CAP].v);
  append_printf(buffer, used, nbytes, "Manage cap: %s\n", a->caps[MANAGE_CAP].v);

  switch (a->type) {
    case IBP_BYTEARRAY: append_printf(buffer, used, nbytes, "Type: IBP_BYTEARRAY\n"); break;
    case IBP_BUFFER:    append_printf(buffer, used, nbytes, "Type: IBP_BUFFER\n"); break;
    case IBP_FIFO:      append_printf(buffer, used, nbytes, "Type: IBP_FIFO\n"); break;
    case IBP_CIRQ:      append_printf(buffer, used, nbytes, "Type: IBP_CIRQ\n"); break;
    default:            append_printf(buffer, used, nbytes, "Type: (%d) UNKNOWN TYPE!!!! \n", a->type); break;
  }

  switch (a->reliability) {
    case ALLOC_HARD: append_printf(buffer, used, nbytes, "Reliability: IBP_HARD\n"); break;
    case ALLOC_SOFT: append_printf(buffer, used, nbytes, "Reliability: IBP_SOFT\n"); break;
    default: append_printf(buffer, used, nbytes, "Reliability: (%d) UNKNOWN RELIABILITY!!!\n", a->reliability);
  }

  append_printf(buffer, used, nbytes, "Current size: " LU "\n", a->size);
  append_printf(buffer, used, nbytes, "Max size: " LU "\n", a->max_size);
  append_printf(buffer, used, nbytes, "Read pos: " LU "\n", a->r_pos);
  append_printf(buffer, used, nbytes, "Write pos: " LU "\n", a->w_pos);
  append_printf(buffer, used, nbytes, "Read ref count: %u\n", a->read_refcount);
  append_printf(buffer, used, nbytes, "Write ref count: %u\n", a->write_refcount);

  if (a->is_alias) {
     append_printf(buffer, used, nbytes, "Alias offset: " LU "\n", a->alias_offset);
     append_printf(buffer, used, nbytes, "Alias size: " LU "\n", a->alias_size);
     append_printf(buffer, used, nbytes, "Alias ID: " LU "\n", a->alias_id);
  ...
  append_printf(buffer, used, nbytes, "\n");
  blank_chksum_set(&cs);
  if (chksum_valid_type(cs_type) == 1) {
  ...
  append_printf(buffer, used, nbytes, "\n");
  append_printf(buffer, used, nbytes, "Read history (slot=%d) (epoch, time, host, id, offset, size)\n", h->read_slot);
  append_printf(buffer, used, nbytes, "---------------------------------------------\n");
  print_rw_history(buffer, used, nbytes, h->read_ts, h->read_slot);

  append_printf(buffer, used, nbytes, "\n");
  append_printf(buffer, used, nbytes, "Write history (slot=%d) (epoch, time, host, id, offset, size)\n", h->write_slot);
  append_printf(buffer, used, nbytes, "---------------------------------------------\n");
  print_rw_history(buffer, used, nbytes, h->write_ts, h->write_slot);

  append_printf(buffer, used, nbytes, "\n");
  append_printf(buffer, used, nbytes, "Manage history (slot=%d) (epoch, time, host, id, cmd, subcmd, reliability, size, expiration_epoch, expiration)\n", h->manage_slot);
  append_printf(buffer, used, nbytes, "---------------------------------------------\n");
