...
int ipdecstr2address(char *src, char *dest)
{
  ...
  number = string_token(srcdup, ".", &bstate, &fin);
  while ((number != NULL) && (j<16)) {
     dest[j] = atoi(number);
     j++;
     number = string_token(NULL, ".", &bstate, &fin);
  }
...
subnet_list_t *new_subnet_list(char *list_string)
{
   ...
   char *list[100];
   n = 0;
   list[n] = string_token(list_str, ";", &mstate, &fin);
   while (strcmp(list[n], "") != 0) {
     n++;
     list[n] = string_token(NULL, ";", &mstate, &fin);
   }
   ...
   if (strcmp(list[0], "open") == 0) {  //** Accept any connections
      ...
      for (i=0; i<sn->n; i++) {  //** Process each element
         ...
         mask = string_token(acl_text, "/", &mstate, &fin);
         ...
         //** Now get the "class"
         s->bits = atoi(string_token(NULL, "/", &mstate, &fin));
