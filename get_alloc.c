...
int parse_cap(char *cap, char **host, int *port, char **rid, char **key, int *ktype)
{
  ...
  char *ptr;
  ptr = string_token(cap, "/", &bstate, &finished); //** gets the ibp:/
log_printf(15, "1 ptr=%s\n", ptr);
  ptr = string_token(NULL, ":", &bstate, &finished);
...
log_printf(15, "2 ptr=%s\n", ptr);
  ...
  *key = string_token(NULL, "/", &bstate, &finished);
  ...
  temp = string_token(NULL, "/", &bstate, &finished);
...
log_printf(15, "get_alloc: after while loop\n");
  ...
  if (ns == NULL) return(-1);
  set_net_timeout(&dt, max_wait, 0);

  nblocks = atoi(string_token(NULL, " ", &bstate, &err));
  state = atoi(string_token(NULL, " ", &bstate, &err));
  cs_type = atoi(string_token(NULL, " ", &bstate, &err));
  header_bs = atoi(string_token(NULL, " ", &bstate, &err));
  bs = atoi(string_token(NULL, " ", &bstate, &err));
  ndata = atoi(string_token(NULL, " ", &bstate, &err));
log_printf(15, "parsed: nb=%d s=%d cs_type=%d hbs=" I64T " bs=" I64T " ndata=%d\n", nblocks, state, cs_type, header_bs, bs, ndata);
  ...
  err = read_netstream_block(ns, to, (char *)&a, sizeof(a));
  if (err != NS_OK) {
