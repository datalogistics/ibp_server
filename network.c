...
int _write_netstream(NetStream_t *ns, const char *buffer, int bsize, Net_timeout_t timeout, int dolock)
{
   ...
   if ((ns_write_chksum_state(ns) == 1) && (total_bytes > 0)) {  //** We have chksumming enabled
      chksum_add(&(ns->write_chksum.chksum), total_bytes, buffer);
//      (&(ns->write_chksum.chksum))->add((&(ns->write_chksum.chksum))->state, total_bytes, buffer);
//      ns->write_chksum.chksum.add(ns->write_chksum.chksum.state, total_bytes, buffer);
...
//        debug_printf(10, "! * nbytes =%d\n", total_bytes); flush_debug();
   ...
   if ((ns_read_chksum_state(ns) == 1) && (total_bytes > 0)) {  //** We have chksumming enabled
      chksum_add(&(ns->read_chksum.chksum), total_bytes, buffer);
...
{
   return(_read_netstream(ns, buffer, size, timeout, 1));
}
...
int readline_netstream_raw(NetStream_t *ns, char *buffer, int bsize, Net_timeout_t timeout, int *status)
{
   ...
   *status = 0;
   ...
   if (finished == 0) {
      ...
      log_printf(15, "readline_netstream_raw: ns=%d Command : !", ns->id);
      for (i=0; i< nbytes; i++) slog_printf(15, "%c", ns->buffer[i]);
      slog_printf(15, "! * nbytes =%d\n", nbytes); flush_debug();
