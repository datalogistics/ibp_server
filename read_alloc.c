...
//log_printf(0, "read_alloc: afile=%s i=%d\n", afile, i);
  ...
  if ((print_blocks == 1) && (chksum_valid_type(cs_type) == 1)) {
     printf("\n\n");
     printf(" Block      Bytes    State          Chksum (calculated if error) \n");
     printf("-------  ----------  -----   --------------------------------------------\n");
     chksum_set(&cs, cs_type);
     bin_len = chksum_size(&cs, CHKSUM_DIGEST_BIN);
     hex_len = chksum_size(&cs, CHKSUM_DIGEST_HEX);
...
//nblocks = 1;
     ...
     for (i=0; i<n; i++) {
        ...
        for (j=0; j<k; j++) {
            ...
            bytes_used = blen[j];
            s = good_block[j];
            convert_bin2hex(bin_len, (unsigned char *)&(block_chksum[j*bin_len]), hex_digest);
            if (s == 0) {
               printf("%7d  %10lld   %2d     %s\n", b, bytes_used, s, hex_digest);
            } else {
               convert_bin2hex(bin_len, (unsigned char *)&(calc_chksum[j*bin_len]), hex_digest2);
