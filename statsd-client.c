...
statsd_link *statsd_init_with_namespace(const char *host, int port, const char *ns_, const char * postfix_)
{
    if (!host || !port)
        return NULL;
    statsd_link *temp = statsd_init(host, port);
    if (ns_) {
        size_t len_ns = strlen(ns_);
        
        if ( (temp->ns = malloc(len_ns + 2)) == NULL ) {
            perror("malloc");
            return NULL;
        }
        strcpy(temp->ns, ns_);
    ...
    if (postfix_) {
        size_t len_postfix = strlen(postfix_);
        
        if ( (temp->postfix = malloc(len_postfix + 2)) == NULL ) {
            perror("malloc");
            return NULL;
        }
        temp->postfix[0] = '.';
        strcpy(temp->postfix + 1, postfix_);
        temp->ns[len_postfix + 1] = 0;
    } else {
        if ( (temp->postfix = malloc(1)) == NULL ) {
            perror("malloc");
            return NULL;
        }
        temp->postfix[0] = 0;
...
void statsd_prepare(statsd_link *link, char *stat, size_t value, const char *type, float sample_rate, char *message, size_t buflen, int lf)
{
    ...
    cleanup(stat);
    if (sample_rate == 1.0) {
        snprintf(message, buflen, "%s%s%s:%zd|%s%s", link->ns ? link->ns : "", stat, link->postfix ? link->postfix : "", value, type, lf ? "\n" : "");
    } else {
        snprintf(message, buflen, "%s%s%s:%zd|%s|@%.2f%s", link->ns ? link->ns : "", stat, link->postfix ? link->postfix : "", value, type, sample_rate, lf ? "\n" : "");
    }
}
